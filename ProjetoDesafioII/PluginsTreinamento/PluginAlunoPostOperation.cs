...
{
    public class PluginAlunoPostOperation : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            try //tentativa de execução
            {
                ...
                if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
                {
                        ...
                        if (!entidadeContexto.Contains("dio_linkedin")) // verifica se o atributo websiteurl não está presente no contexto
                        {
                            throw new InvalidPluginExecutionException("Campo Linkedin é obrigatório!"); // exibe Exception de Erro
                        }
                        if (String.IsNullOrEmpty(entidadeContexto["dio_linkedin"].ToString()))
                        {
                            throw new InvalidPluginExecutionException("Campo Linkedin é obrigatório!"); // exibe Exception de Erro
                        ...
                        Task.Attributes["dio_taskalunoid"] = new EntityReference("dio_aluno", context.PrimaryEntityId);
                        Task.Attributes["subject"] = "Visite  o Linkedin: " + entidadeContexto["dio_linkedin"];
